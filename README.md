...
- **通知**：每个平台签到结束都会推送结果。**优先调用青龙自带 `send_notify`**（复用你在青龙面板配好的通道，和"仓库变动"通知走同一条线，无需再单独配）；非青龙环境（本地调试）才走脚本内直推（Server酱/Pushplus/Bark/企业微信/钉钉，需自备变量）兜底。设置 `CT_DISABLE_NOTIFY=1` 可只打印不推送。
