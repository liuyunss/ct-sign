...
| `CT_RANDOM_DELAY` | 随机延迟上限（秒）：每个账号签到前随机休眠，模拟用户随机触发，防固定节奏被识别 | `5` |
| `CT_RANDOM_DELAY_MIN` | 随机延迟下限（秒）；上限<下限时自动取等于上限 | `2` |
| `CT_CRON` | 自动建任务的 cron（默认 `0 8 * * *`） | `30 7 * * *` |
...
### 随机延迟（模拟用户随机触发）
- 即便青龙定时任务是固定时刻（如每天 08:00），脚本也会在**每个账号签到前**随机休眠 `[CT_RANDOM_DELAY_MIN, CT_RANDOM_DELAY]` 秒，使访问节奏不规律，更像真人。
