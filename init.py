"""init.py：读取所有平台，在青龙里自动创建定时任务（可选手动工具）。

【推荐】日常使用请直接用 ql repo 白名单 sign_ 订阅（见 README），青龙会把每个
sign_<平台>.py 直接建成一行定时任务，无需本脚本、也不依赖青龙 API 令牌。

本脚本仅在「不用 ql repo、想手动建任务」时备用：容器内会自动获取青龙内部令牌
免 key 建任务；容器外接不到令牌则只校验配置并提示。

任务计划（cron）可用环境变量 CT_CRON 覆盖，默认每天 00:01（1 0 * * *）。
任务命名统一前缀「CT-Sign」+ 中文平台名；并清理 ql repo 误建的野任务。
"""
